...
if(isset($_POST['saveCustomName'])){
    ...
    if(empty($error)){
        ...
        $SQL = $db->prepare("UPDATE `pterodactyl_servers` SET `custom_name` = :custom_name WHERE `id` = :id");
    ...
    <div class="d-flex flex-column-fluid">
        <div class="container">
            <div class="row">
                ...
                <?php
                $SQL = $db->prepare("SELECT * FROM `pterodactyl_servers` WHERE `user_id` = :user_id AND `deleted_at` IS NULL ORDER BY `id` DESC");
                                        ...
                                        <div class="col">
                                            <h3 class="mb-0">
                                                ...
                                                <?php if(is_null($row['custom_name'])){ ?>
                                                    <b>Gameserver</b> #<?= $row['id']; ?> <span data-toggle="tooltip" title="Eigenen Produkt-Namen setzen"><i style="cursor:pointer;" data-toggle="modal" data-target="#productModal<?= $row['id']; ?>" class="far fa-edit"></i></span>
                                                <?php } else { ?>
                                ...
                                <div class="card-footer">
                                    <div class="row align-items-center text-center">
                                        <div class="col-md-6">
                                            ...
                                            <?php } else { ?>
                                                <a href="<?= env('URL'); ?>manage/gameserver/<?= $row['id']; ?>" class="btn btn-block btn-outline-primary">
                                                    <b>Verwalten</b>
                                                </a>
                                            <?php } ?>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <?php } else { ?>
                                                <a href="<?= env('URL'); ?>renew/gameserver/<?= $row['id']; ?>" class="btn btn-block btn-outline-primary">
                                                    <b>Verlängern</b>
                                                </a>
                                            <?php } ?>
                                        </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <?php } ?>
                                <h1 class="alert-heading">
                                    <br>
                                    Du hast aktuell noch keinen Gameserver 😲
                                </h1>
                                <br>
                                <h4>
                                    Miete dir jetzt einen Prepaid Gameserver bei uns!
                                </h4>
                                <br>
                                <p>
                                    <a href="<?= env('URL'); ?>order/gameserver" class="btn btn-outline-primary text-uppercase font-weight-bolder pulse-red">Jetzt bestellen</a>
                                </p>
                            </div>
                        </div>
                    <?php } ?>

    ...
    <div class="d-flex flex-column-fluid">
        <div class="container">
            <div class="row">
                ...
                <?php
                $SQL = $db->prepare("SELECT * FROM `vm_servers` WHERE `user_id` = :user_id AND `deleted_at` IS NULL AND `type` = 'LXC' ORDER BY `id` DESC");
                $SQL->execute(array(":user_id" => $userid));
                if ($SQL->rowCount() != 0) {
                    while ($row = $SQL -> fetch(PDO::FETCH_ASSOC)){
                        ...
                        <div class="col-12 col-xl-3">
                            <div class="card text-center shadow mb-5">
                                        ...
                                        <div class="col">
                                            <h3 class="mb-0">
                                                ...
                                                <?php if(is_null($row['custom_name'])){ ?>
                                                    <b>vServer</b> #<?= $row['id']; ?> <span data-toggle="tooltip" title="Eigenen Produkt-Namen setzen"><i style="cursor:pointer;" data-toggle="modal" data-target="#productModal<?= $row['id']; ?>" class="far fa-edit"></i></span>
                                                <?php } else { ?>
                                ...
                                <div class="card-footer">
                                    <div class="row align-items-center text-center">
                                        <div class="col-md-6">
                                            <a href="<?= env('URL'); ?>manage/vserver/<?= $row['id']; ?>" class="btn btn-block btn-outline-primary">
                                                <b>Verwalten</b>
                                            </a>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <a href="<?= env('URL'); ?>renew/vserver/<?= $row['id']; ?>" class="btn btn-block btn-outline-primary">
                                                <b>Verlängern</b>
                                            </a>
                                        </div>
                                    </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <?php } ?>
                                <h1 class="alert-heading">
                                    <br>
                                    Du hast aktuell noch keinen vServer 😲
                                </h1>
                                <br>
                                <h4>
                                    Miete dir jetzt einen Prepaid LXC vServer bei uns!
                                </h4>
                                <br>
                                <p>
                                    <a href="<?= env('URL'); ?>order/vserver" class="btn btn-outline-primary text-uppercase font-weight-bolder pulse-red">Jetzt bestellen</a>
                                </p>
                            </div>
                        </div>
                    <?php } ?>

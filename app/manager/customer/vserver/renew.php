...
$SQLGetServerInfos = $db->prepare("SELECT * FROM `vm_servers` WHERE `id` = :id AND `type` = 'LXC'");
...
if(isset($_POST['renew'])){
    ...
    if(empty($error)){
        ...
        $user->addTransaction($userid,'-'.$price,'vServer Verlängerung');

        echo sendSuccess('Dein vServer wurde verlängert');
